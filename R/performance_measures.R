...
performance_measures = function(list_objects, eval_metric, sort = list(variable = 'Median', decreasing = TRUE)) {
  ...
  complete_cases_tr = lapply(tmp_params, function(x) complete.cases(t(do.call(cbind, lapply(x, function(y) y[, 1])))))
  complete_cases_te = lapply(tmp_params, function(x) complete.cases(t(do.call(cbind, lapply(x, function(y) y[, 2])))))
